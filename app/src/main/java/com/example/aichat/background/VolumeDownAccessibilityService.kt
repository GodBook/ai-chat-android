...
class VolumeDownAccessibilityService : AccessibilityService() {
    ...
    private val keyHandler = ScreenshotKeyHandler()
    ...
    override fun onKeyEvent(event: KeyEvent): Boolean {
        val key = when (event.keyCode) {
            KeyEvent.KEYCODE_VOLUME_DOWN -> ScreenshotKeyHandler.Key.DOWN
            KeyEvent.KEYCODE_VOLUME_UP -> ScreenshotKeyHandler.Key.UP
            else -> return false
        }
        if (event.action != KeyEvent.ACTION_DOWN && event.action != KeyEvent.ACTION_UP) return false
        val now = android.os.SystemClock.elapsedRealtime()
        val result = keyHandler.handle(
            key = key,
            ...
            enabled = enabled,
            trigger = trigger,
            speechActive = app.container.ttsManager.playbackState.value.let { it.isPlaying || it.isPaused },
        )
        if (result.stopSpeech) app.container.ttsManager.stop()
        if (result.capture && now - lastTriggerAt >= TRIGGER_DEBOUNCE_MS) {
            lastTriggerAt = now
            captureFromTrigger()
        }
