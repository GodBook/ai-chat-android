...
当前版本：**1.6.8（versionCode 31）**。更新说明见 [release-notes-1.6.8.md](release-notes-1.6.8.md)。
...
- **联网搜索**：并行检索 360、国内 Bing 与国际 Bing，合并去重、核对相对日期、筛除明确过期及无关摘要；天气问题优先使用 Open-Meteo 结构化数据，标明地点、时区、数据时刻，区分当前模型估算和逐日预报。来源卡片支持核验，检索失败明确提示，重试与重新生成会刷新资料
...
.\publish.ps1 -VersionName "1.6.8" -NotesFile "release-notes-1.6.8.md"
```
